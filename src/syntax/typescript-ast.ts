import {
  createSourceFile, SyntaxKind, isToken,
  SourceFile, Node, ScriptTarget
} from 'typescript'
...
class TypeScriptParser {
  ...
  parse(program: string) {
    var src = createSourceFile('this-program.ts', program,
      ScriptTarget.Latest)
  ...
  postprocessAst(u: Node, src: SourceFile) {
    ...
    if (isToken(u)) {
      ...
      var children: any = u.getChildren(src).map(s => this.postprocessAst(s, src))
