...
export function* lazyFlatMap<T, TResult>(
  arr: Iterable<T>,
  map: (obj: T) => Generator<TResult>
): Generator<TResult> {
  if (!arr) {
    return;
  }
...
export function toSubtreeString(
  vertex: Vertex,
  sep = " ",
  includeSyntaxTokens = false
): string {
  const allTokens = toSubtreeStringVertexGen(vertex);
  const relevantTokens = includeSyntaxTokens
    ? allTokens
    : lazyFilter(allTokens, Syntax.isNonSyntaxToken);
