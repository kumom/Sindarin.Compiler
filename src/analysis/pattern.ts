...
type Route<VData> = Array<Vertex<VData>>
...
class HMatcher<VData = any> {
  ...
  private _matched(genf: () => Generator<Edge>): Matched {
  ...
  resolvePatternDefinitions(pattern: HMatcher.RoutePatternDefinition, handler: (route: Route<VData>) => void) {
    ...
    const startingSet = (vertex != null) ? this.v(vertex) : this.l(labelPred)
    const vertexLabelPat = (vertex != null) ? vertex.label : undefined
...
namespace HMatcher {
  ...
  export function toLabelPred(l: LabelPat): LabelPred {
    ...
    else if (l instanceof RegExp) return (s: string) => !(s.match(l) == null)
    else return l
  }
  ...
  export class Matched<VData = any> {
    ...
    e(cont: (e: Edge) => void | boolean) { // iterates edges
      for (const e of this.gen) cont(e)
    }
    ...
    s(cont: (t: Vertex<VData>) => void | boolean, labelPat?: LabelPat) { // iterates edge sources
      const p = !labelPat ? LANY_LABEL : toObjectWithLabel(labelPat)
      this.e(e => {
        for (const u of lazyFilter<Vertex>(e.sources, p)) cont(u)
      })
    }
    ...
    resolvePatternDefinitions(handler: (route: Array<Vertex<VData>>) => void, definitions: PatternDefinition[], payload: PatternDefinitionPayload, route?: Route<VData>): boolean {
      const [nextDefinition, ...restOfDefinitions] = definitions && (definitions.length > 0) ? definitions : [null]
      if (nextDefinition == null) return false

      const { labelPred, vertex, through, modifier, excluding } = nextDefinition

      const methodBase = through ? THROUGH_TO_METHOD[through] : undefined
      ...
      const routeHandler = (u: any) => {
        if (found && firstOnly) {
          return
        }

        const extendedRoute = (route != null) ? [...route, u] : [u]
