...
class Hypergraph<VData = any> {
  ...
  get nodes() {
    ...
    for (const e of this.edges) {
      for (const u of e.incident) { v.add(u) }
    }
    return v
  }
  ...
  add(edges: Hypergraph.EdgeData[]) {
    ...
    var get = (u: Hypergraph.VertexId | Hypergraph.Vertex<VData>) => {
      if (typeof u === 'number') {
        if (u > 0) return this._get(u)
        else {
          var v = vmap.get(u)
          if (v == null) { v = this._fresh(); vmap.set(u, v) }
          return v
        }
      } else {
        if (this.vertices.get(u.id) == null) { this.vertices.set(u.id, u) }
        return u
      }
    }
  ...
  merge(vertices: Array<Hypergraph.Vertex<VData>>) {
    var rep = vertices[0]
  ...
  fromAst(ast: Ast) {
    var self = this; var c: Hypergraph.VertexId = this._max
    function aux(ast: Ast) {
      var root = ++c; var u = self._get(root)
  ...
  toVis(edgeNodeProfile = null): HypergraphView {
...
namespace Hypergraph {
  ...
  export class Edge {
    ...
    toVis(edgeNodeProfile: any) {
      ...
      // @ts-expect-error
      var edges: Edge[] = [{ from: nucleus, to: this.target.id, ...TO },
      // @ts-expect-error
...
const TO = { arrows: { to: { enabled: true, scaleFactor: 0.5 } }, color: '#997', length: 1 }
const FROM = { arrows: { middle: { enabled: true, scaleFactor: 0.5 } }, color: '#997', length: 1 }
const LIT = { color: '#9d9', shapeProperties: { borderRadius: 0 } }
const FAINT = {
  color: {
    background: '#ddd',
    border: '#bbb',
...
class HypergraphView {
  ...
  constructor(peg: Hypergraph, data: HypergraphView.Data) {
    this.peg = peg
    this.data = data
    if (this.data.options != null) { this.options = this.data.options }
  }
  ...
  merge(that: HypergraphView) {
    that.data.nodes.forEach(u => {
      if (this.data.nodes.get(u.id!) == null) { this.data.nodes.add(u) }
    })
  ...
  untangle() {
    for (const row of this.iterLevels()) { this.sortHorizontally(row!) }
  }
  ...
  * iterLevels() {
  ...
  postprocess() {
    for (const e of Object.values(this._edges)) { this._shortenEdge(e) }
    this.network.redraw()
  }
  ...
  _shortenEdge(e: Edge) {
    ...
    if (shape.constructor.name === 'StraightEdge') {
      shape._drawLine = function (ctx: CanvasRenderingContext2D, val: any) {
